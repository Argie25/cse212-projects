...
public static class Recursion
{
    ...
    {
        // Base case: if the built word has the desired size, store it.
    if (word.Length == size)
    {
        results.Add(word);
        return;
    ...
    for (int i = 0; i < letters.Length; i++)
    {
        ...
        // Recurse with c added to the permutation
    ...
    // Recursive case:
    // Replace the '*' with 0 and then with 1
    ...
    {
        // Initialize path list only on the first call
    if (currPath == null)
    {
        currPath = new List<ValueTuple<int, int>>();
    }

    // If the move is not valid, stop exploring this path
    ...
    // If we've reached the end, record this successful path
    if (maze.IsEnd(x, y))
    {
    ...
    // Explore in 4 possible directions: Right, Down, Left, Up
